"""
Training script for DDM baseline.
...
from src.evaluate import evaluate_ddm
from src.models import DDM
...
def get_args():
    parser = argparse.ArgumentParser(description="DDM Baseline Training")
    parser.add_argument("--config", type=str, default="configs/MUTAG.yaml")
    parser.add_argument("--output_dir", type=str, default="logs/ddm")
...
def main():
    ...
    for run_i, seed in enumerate(cfg.seeds):
        ...
        for epoch in range(cfg.SOLVER.MAX_EPOCH):
            ...
            if (epoch + 1) % 1 == 0 and epoch > 1:
                model.eval()
                f1 = evaluate_ddm(
                    model, cfg.eval_T, pooler, eval_loader, device, logger
                )
                if f1 > best_f1:
